...
export const SeasonFourRostersPage: React.FC = () => {
  ...
  const renderEmptySlots = (currentPlayers: number) => {
    ...
    for (let i = currentPlayers; i < 5; i++) {
      emptySlots.push(
        <div key={i} className="h-[76px] p-2 bg-idl-gray rounded-lg border border-dashed border-idl-dark">
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="text-center text-idl-light py-8">No teams registered yet</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {teams.map((team) => (
            <div key={team.id} className="bg-idl-gray rounded-lg shadow-md p-4 transition-colors">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <h2 className="text-lg font-bold text-idl-light cursor-default" title={team.name}>
                ...
                <span className="px-2 py-1 text-xs rounded-full bg-idl-accent text-white">Div {team.division_id}</span>
              </div>

              <div className="space-y-2">
                {team.players &&
                  team.players.map((player, index) => (
                    <div key={index} className="h-[76px] p-2 bg-idl-gray rounded-lg border border-idl-dark">
                      <div className="flex justify-between items-center">
                        <div className="space-y-0.5">
                          <p className="font-medium text-lg text-idl-light">{player.name}</p>
                          <a
