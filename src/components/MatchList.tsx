...
export const MatchList: React.FC = () => {
  ...
  const getMatchesForWeek = (week: number) => {
    const div1Matches = divisionMatches[1].filter(
      (match) => match.week === week
    );
    const div2Matches = divisionMatches[2].filter(
      (match) => match.week === week
    );
    const div3Matches = divisionMatches[3].filter(
      (match) => match.week === week
    );
  ...
  const { div1Matches, div2Matches, div3Matches, weekDate } =
    getMatchesForWeek(currentWeek);
  ...
  const getScoreColor = (score: number[], index: number) => {
    ...
    if (
      (index === 0 && score[0] > score[1]) ||
      (index === 1 && score[1] > score[0])
    ) {
    ...
    <div key={match.id} className="relative overflow-x-scroll md:overflow-auto">
      {match.isByeWeek ? (
        ...
        <button
          ...
          className="w-full p-2 md:p-4 hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors relative group text-left"
        >
          ...
          <div className="relative md:px-4 py-4 md:py-0">
            ...
            <div className="md:absolute mt-4 md:mt-0 flex justify-center relative md:right-2 md:top-1/2 md:-translate-y-1/2">
              <div className="w-8 h-8 rounded-full bg-[#1d1d1b] flex items-center justify-center hover:opacity-90 transition-all">
                <ChevronDown
                  size={20}
                  className={`text-[#46ffd0] transform transition-transform ${
      ...
      <div
        className={`overflow-hidden transition-all duration-300 ${
          expandedMatch === match.id ? "max-h-fit" : "max-h-0"
        }`}
      >
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            {!match.isByeWeek && match.games && (
              <>
                {/* Game 1 */}
                <div>
                  <h4 className="text-lg font-medium text-gray-600 dark:text-gray-300 mb-3 text-center">
                    Game 1
                  </h4>
                  <MatchStats
                    matchId={match.id}
